...
type CandlestickChartPropsWithOptionalSetter = Omit<
  CandlestickChartProps,
  "setLiveInterval"
> & {
  setLiveInterval?: (interval: string) => void;
};
...
const CandlestickChart = ({
  ...
  liveOhlcv = null,
  mode = "historical",
  liveInterval: propLiveInterval,
  setLiveInterval: propSetLiveInterval,
}: CandlestickChartPropsWithOptionalSetter) => {
  const chartContainerRef = useRef<HTMLDivElement | null>(null);
  const chartRef = useRef<IChartApi | null>(null);
  const candleSeriesRef = useRef<ISeriesApi<"Candlestick"> | null>(null);
  const prevOhlcDataLength = useRef<number>(data?.length || 0);

  const [internalLiveInterval, setInternalLiveInterval] = useState(
    propLiveInterval || "1s",
  );
  const isControlled = propSetLiveInterval !== undefined;
  const liveInterval = isControlled ? propLiveInterval : internalLiveInterval;
  const setLiveInterval = isControlled
    ? propSetLiveInterval
    : setInternalLiveInterval;
